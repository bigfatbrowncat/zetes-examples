...
namespace cubex
{
	...
	void MeshBuffer::process(const string& vertexPositionVec3ShaderVariableName,
							  const string& normalVec3ShaderVariableName,
							  const string& textureCoordsVec2ShaderVariableName) const
	{
		const ShaderProgram* activeSP = ShaderProgram::getActive();
		draw(activeSP->getAttribLocation(vertexPositionVec3ShaderVariableName),
			 activeSP->getAttribLocation(normalVec3ShaderVariableName),
			 activeSP->getAttribLocation(textureCoordsVec2ShaderVariableName));
	}
